...
filenameStats = 'LaLiga_V5_' + d.now().strftime('%Y_%m_%d_%H_%M%S') + '.xlsx'
statsBook = Workbook()
wsheet = statsBook.create_sheet('La Liga Refs')
ws = statsBook.active

wsheet.cell(row=1, column=1, value='La Liga')
...
class footballStatsScraper:
    ...
    def getGoalDistribution(self):
        ...
        oddRowElem = self.browser.find_element_by_css_selector(".odd")
        evenRowElem = self.browser.find_element_by_css_selector(".even")
        dataTable = self.browser.find_element_by_css_selector('table.items')
        clubStats = []
        clubs = []

        logger.info('La Liga Goal Distribution')
        ...
        for club in clubs:
            ...
            #    if cm == 0:
            #        sheet.write(rw, 1, data)
            #    elif cm == 1:
            #        sheet.write(rw, cm + 1, data)
            #   cm += 1
            rw += 1
            # sheet.write(rw, cm, club[rw])
    ...
    def getOwnGoals2(self, wb):
        ...
        self.browser.get("https://www.transfermarkt.com/laliga/eigentorstatistik/wettbewerb/ES1/plus/?saison_id=2019")
        #dataTable = self.browser.find_element_by_css_selector('table.items')
        #dataTable = self.browser.find_element_by_xpath('//div[contains(string(), "Causer")]/table')
        ...
        for x in range(3, len(qColumn)):
            if qColumn[x].value is None:
                ws.cell(row=x + 1, column=10).value = int('0')
                logger.info('Cell value: ' + str(ws.cell(row=x + 1, column=10).value))
    ...
    def getCorrectScores(self, wb):
        ...
        time.sleep(4)
        self.browser.get(
            "https://docs.google.com/spreadsheets/d/e/2PACX-1vRzC-0OUWqFuNiIR2qn8hatt2SFdmfnrVJJ53CUBdcDrdMnQ28nMX_Ep-OXa2HRj-kDoIR3jEefzBpD/pubhtml?gid=78603189&single=true&widget=false&headers=false&chrome=false")
        ...
        ca= 13
        for h in headers:
            #mycell = ws.cell(3, column=cc)
            #mycell = he
            logger.info('Writing header: ' + str(h) + ' at row 3 col ' + str(ca))
            ws.cell(row=3, column=ca).value = h
            ca += 1
    ...
    def getMatchesPlayed(self, wb):
        # Add Additional column for Own Goals
        self.browser.get("https://www.transfermarkt.com/primera-division/spieltagtabelle/wettbewerb/ES1/saison_id/2019")
        dataTable = self.browser.find_element_by_xpath('//div[contains(string(), "Table LaLiga 19/20")]/table')
        teams = []
        hders = []
        logger.info('La Liga Matches Played')
        ct = 0
        ...
        # Add headers to excel file
        cc = 42
        logger.info('Headers for Matches Played: ' + str(hders))
        for h in hders:
            wss.cell(row=3, column=cc).value = h
            logger.info('Writing header: ' + str(h) + ' at row 3 col ' + str(cc))
            cc += 1
            ...
            for c in teams:
                if ctt < 4:
                    ...
                    if first_column[x].value is None:
                        ...
                        # TODO changed from 0 to 1 for testing
                        if c[1] in first_column[x].value or \
                                (c[1] == 'Atl. Madrid' and first_column[x].value == 'Atlético Madrid') or \
                                (c[1] == 'Ath. Bilbao' and first_column[x].value == 'Athletic Bilbao') or \
                                (c[1] == 'Alaves' and first_column[x].value == 'Deportivo Alavés') or \
                                (c[1] == 'Celta Vigo' and first_column[x].value == 'Celta de Vigo') or \
                                (c[1] == 'Leganes' and first_column[x].value == 'CD Leganés'):
                            # If team is in ownGoalStats, add the team stats to table in Excel
                            for stat in c:
                                # mycell= ws.cell(row=rowCount, column=columnCount)
                                # mycell = stat
    ...
    def getConcededPenalties(self, wb):
        self.browser.get("https://www.statbunker.com/competitions/AgainstPenalty?comp_id=648")
        dataTable = self.browser.find_element_by_css_selector('table.table')
        clubs = []
        header = []
        ct = 0

        logger.info('La Liga Conceded Penalties Goals')
            ...
            for c in clubs:
                if len(c) > 0:
                    ...
                    if first_column[x].value is None:
                        ...
                        if c[0] in first_column[x].value or \
                                (c[0] == 'Atletico Madrid' and first_column[x].value == 'Atlético Madrid') or \
                                (c[0] == 'CD Alaves' and first_column[x].value == 'Deportivo Alavés') or \
                                (c[0] == 'Celta Vigo' and first_column[x].value == 'Celta de Vigo') or \
                                (c[0] == 'CD Leganes' and first_column[x].value == 'CD Leganés'):
                            # If team is in ownGoalStats, add the team stats to table in Excel
                            for stat in c:
                                # TODO: Added in V5
                                if stat == '-':
        ...
        wb.save(filenameStats)
        # statsBook.save(filenameStats)
        self.getRefs("https://www.transfermarkt.com/laliga/schiedsrichter/wettbewerb/ES1/saison_id/2019/plus/1", 0)
    ...
    def getRefDetails(self, url):
        ...
        childElement1 = self.browser.find_elements_by_xpath('//img[@alt="LaLiga"]')
            ...
            if len(ref) != 0 and len(ref) != 1:
                ...
                for col in range(minCol, maxCol - 1):
                    ...
                    if ref[track] == '-':
                        ...
                        logger.info(' wsheet.cell(row=' + str(rw) + ', column=' + str(col) + '.value = ' + str(ref[track]))
                    track += 1
                rw += 1
    ...
    def getRefs(self, url, pgCount):
        ...
        for row in dataTable.find_elements_by_css_selector('tr'):
            ...
            for cell in row.find_elements_by_tag_name('td'):
                ...
                if cell.text != '':
                    logger.info(
                        'In normal info statement - Count: ' + str(count) + ' Appending ' + str(
                            cell.text.rstrip))
                    # logger.info('cell: ' + str(cell.text.rstrip()))
