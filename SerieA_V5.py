...
available = []
# max of 25 refs per page
pageCount = 0
filenameStats = 'SerieA_V5_' + d.now().strftime('%Y_%m_%d_%H_%M%S') + '.xlsx'

# Create new workbook
statsBook = Workbook()
statsBook.create_sheet('Serie A Refs')
wsheet = statsBook['Serie A Refs']
ws = statsBook.active


# wb = load_workbook('FootballStatsSerieA_2020_02_24_20_5523.xlsx')
# ws = wb.active
...
class footballStatsScraper:
    ...
    def mainFunction(self):
        # self.statsBook = load_workbook(filenameStats)
        # self.statsBook = Workbook()
        # self.statsBook.create_sheet('La Liga')
        # wsheet = self.statsBook['La Liga']
        ...
        self.getGoalDistribution(statsBook)
    ...
    def getGoalDistribution(self, wb):
        self.browser.get("https://www.transfermarkt.com/serie-a/torverteilungart/wettbewerb/IT1/plus/1")
        dataTable = self.browser.find_element_by_css_selector('table.items')
        clubStats = []
        clubs = []

        logger.info('Serie A Goal Distribution')
        ...
        self.getOwnGoals2(wb)
        # sys.exit()

    def getOwnGoals2(self, wb):
        # Add Additional column for Own Goals
        self.browser.get("https://www.transfermarkt.com/serie-a/eigentorstatistik/wettbewerb/IT1")
        ...
        ww.cell(row=3, column=cc).value = 'Own Goals'
                ...
                if ctt < 1:
                    ...
                    if first_column[x].value is None:
                        ...
                        if c[0] in first_column[x].value:
                            ...
                            ww.cell(row=x + 1, column=columnCount).value = int(c[1])
                            logger.info('Cell value: ' + str(ww.cell(row=x + 1, column=columnCount).value))
        ...
        for x in range(3, len(qColumn)):
            if qColumn[x].value is None:
                ww.cell(row=x + 1, column=10).value = int('0')
                logger.info('Cell value: ' + str(ww.cell(row=x + 1, column=10).value))
    ...
    def getCorrectScores(self, wb):
        ...
        time.sleep(4)
        self.browser.get(
            "https://docs.google.com/spreadsheets/d/e/2PACX-1vSGY6_1bhBhDMQ8D9Xfkprqwb2gPdpEiM3Yb56Snfypsq_aWt2JLqZ3wmcQskrn43yvzqFjn6soXgNR/pubhtml?gid=78603189&single=true&widget=false&headers=false&chrome=false")
        ...
        ct = 0
        for h in headers:
            mycell = ww.cell(3, column=cc)
            mycell = h
            ww.cell(row=3, column=cc).value = h
            cc += 1
            ct += 1
                ...
                if ctt < 4:
                    ...
                    if first_column[x].value is None:
                        ...
                        if c[0] in first_column[x].value or \
                                (c[0] == 'Spal' and first_column[x].value == 'SPAL'):
                            ...
                            for stat in c:
                                ...
                                logger.info('stat[count]: ' + str(stat))
                                logger.info('Row/Column: ' + str(x) + ' ' + str(columnCount))
                                ww.cell(row=x + 1, column=columnCount).value = stat
                                logger.info('Cell value: ' + str(ww.cell(row=x + 1, column=columnCount).value))
                                count += 1
                                columnCount += 1
                        rowCount += 1
    ...
    def getMatchesPlayed(self, wb):
        # Add Additional column for Own Goals
        self.browser.get("https://www.transfermarkt.com/serie-a/spieltagtabelle/wettbewerb/IT1")
        dataTable = self.browser.find_element_by_xpath('//div[contains(string(), "Table Serie A 19/20")]/table')
        teams = []
        hders = []
        logger.info('Serie A Matches Played')
        ct = 0
    ...
    def getConcededPenalties(self, wb):
        # TODO: Changed for V5
        self.browser.get("https://www.statbunker.com/competitions/LeagueTable?comp_id=649")
        ...
        logger.info('Serie A Conceded Penalties Goals')
                ...
                if ctt < 4:
                    ...
                    if first_column[x].value is None:
                        ...
                        if c[1] in first_column[x].value or \
                                (c[1] == 'Spal' and first_column[x].value == 'SPAL') or \
                                (c[1] == 'Internazionale' and first_column[x].value == 'Inter Milan'):
                            # If team is in ownGoalStats, add the team stats to table in Excel
                            for stat in c:
                                # TODO: Added in V5
                                if stat == '-':
        ...
        self.getRefs("https://www.transfermarkt.com/serie-a/schiedsrichter/wettbewerb/IT1/saison_id/2019/plus/1", 0)
        # sys.exit()
    ...
    def getRefDetails(self, url):
        ...
        childElement1 = self.browser.find_elements_by_xpath('//img[@alt="Serie A"]')
    ...
    def getRefs(self, url, pgCount):
        ...
        ct = 0
        # for h in hders:
        #    wsheet.cell(row=3, column=cc).value = h
        #    cc += 1
        #   ct += 1
        ...
        logger.info('Done writing to Excel. Saving...')
        # fname = 'newFootballStatsTest' + d.now().strftime('%Y_%m_%d_%H_%M%S') + '.xlsx'
        # self.statsBook.save(filenameStats)
        statsBook.save(filenameStats)
        # statsBook.save(filenameStats)
        # refNum = 0
        for lnk in links:
...
file_name = 'SerieA' + d.now().strftime('%Y_%m_%d_%H_%M%S') + '.log'
