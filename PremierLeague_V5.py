...
# filenameStats = 'FootballStatsTest' + d.now().strftime('%Y_%m_%d_%H_%M%S') + '.xls'
filenameStats = 'PremierLeague_V5_' + d.now().strftime('%Y_%m_%d_%H_%M%S') + '.xlsx'
statsBook = Workbook()
wsheet = statsBook.create_sheet('Premier League Refs')
ws = statsBook.active

wsheet.cell(row=1, column=1, value='Premier League')
ws.cell(row=1, column=1, value='Premier League')
...
class footballStatsScraper:
    ...
    def getGoalDistribution(self):
        self.browser.get("https://www.transfermarkt.com/premier-league/torverteilungart/wettbewerb/GB1/plus/1")
        dataTable = self.browser.find_element_by_css_selector('table.items')
        clubStats = []
        clubs = []

        logger.info('Premier League Goal Distribution')
    ...
    def getOwnGoals2(self, wb):
        ...
        self.browser.get("https://www.transfermarkt.com/premier-league/eigentorstatistik/wettbewerb/GB1")
        ...
        logger.info('Premier League Own Goals')
    ...
    def getCorrectScores(self, wb):
        ...
        time.sleep(4)
        self.browser.get(
            "https://docs.google.com/spreadsheets/d/e/2PACX-1vRAmcEyWUquECAwbV4MNMsKmnbHZaoyAdYwpEx6k2VymWKsnak5dspwGICW-MNIkCR4AtK3JCatvEy2/pubhtml?gid=78603189&single=true&widget=false&headers=false&chrome=false")
                ...
                if ctt < 4:
                    ...
                    if first_column[x].value is None:
                        ...
                        if c[0] in first_column[x].value or \
                                (c[0] == 'Man City' and first_column[x].value == 'Manchester City') or \
                                (c[0] == 'Man Utd' and first_column[x].value == 'Manchester United') or \
                                (c[0] == 'Wolves' and first_column[x].value == 'Wolverhampton Wanderers') or \
                                (c[0] == 'Sheff Utd' and first_column[x].value == 'Sheffield United'):
    ...
    def getMatchesPlayed(self, wb):
        # Add Additional column for Own Goals
        self.browser.get("https://www.transfermarkt.com/premier-league/spieltagtabelle/wettbewerb/GB1")
        # dataTable = self.browser.find_element_by_xpath('//div[contains(string(), "Table Premier League 19/20")]/table')
        dataTable = self.browser.find_element_by_xpath('//div[contains(string(), "Table Premier League 19/20")]/table')

        teams = []
        hders = []
        logger.info('Premier League Matches Played')
        ct = 0
                ...
                if ctt < 4:
                    ...
                    if first_column[x].value is None:
                        ...
                        if c[1] in first_column[x].value or \
                                (c[1] == 'Man City' and first_column[x].value == 'Manchester City') or \
                                (c[1] == 'Man Utd' and first_column[x].value == 'Manchester United') or \
                                (c[1] == 'Wolves' and first_column[x].value == 'Wolverhampton Wanderers') or \
                                (c[1] == 'Sheffield Utd.' and first_column[x].value == 'Sheffield United') or \
                                (c[1] == 'Spurs' and first_column[x].value == 'Tottenham Hotspur'):
                            # If team is in ownGoalStats, add the team stats to table in Excel
                            for stat in c:
                                # mycell= ws.cell(row=rowCount, column=columnCount)
                                # mycell = stat
    ...
    def getConcededPenalties(self, wb):
        # TODO: Changed to StatsBunker for V5
        self.browser.get("https://www.statbunker.com/competitions/AgainstPenalty?comp_id=639")
        # TODO: Chanegd to table.tablefor V5
        ...
        logger.info('Premier League Conceded Penalties Goals')
            ...
            for col in range(min_col, max_col):
                if ws1.cell(row=row, column=col).value is None:
                    if col == min_col:
                        ...
                        #if row > hold_row > 0:
                        #    logger.info('Adding Newcastle to ' + str(min_row) + str(min_col))
                         #   ws1.cell(row=row, column=col).value = 'Newcastle'
                        #else:
                        #    logger.info('Adding Liverpool to ' + str(min_row) + str(min_col))
                         #   ws1.cell(row=row, column=col).value = 'Liverpool'
                    else:
        ...
        wb.save(filenameStats)
        # statsBook.save(filenameStats)
        self.getRefs("https://www.transfermarkt.com/premier-league/schiedsrichter/wettbewerb/GB1/saison_id/2019/plus/1", 0)
        # self.getConcededPenalties(wb)
        #sys.exit()
    ...
    def getRefDetails(self, url):
        ...
        childElement1 = self.browser.find_elements_by_xpath('//img[@alt="Premier League"]')
    ...
    def getRefs(self, url, pgCount):
        ...
        refs = []
        hders = []
        logger.info('Premier League Refs')
        ct = 0
        count = 0
