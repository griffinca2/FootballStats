...
pageCount = 0
# TODO: Change for each version
filenameStats = 'Bundesliga_V5_' + d.now().strftime('%Y_%m_%d_%H_%M%S') + '.xlsx'
statsBook = Workbook()
wsheet = statsBook.create_sheet('Bundesliga Refs')
ws = statsBook.active

wsheet.cell(row=1, column=1, value='Bundesliga')
...
class Bundesliga:
    ...
    def getGoalDistribution(self):
        self.browser.get("https://www.transfermarkt.com/bundesliga/torverteilungart/wettbewerb/L1/plus/1")
        dataTable = self.browser.find_element_by_css_selector('table.items')
        clubStats = []
        clubs = []

        logger.info('Bundesliga Goal Distribution')
    ...
    def getOwnGoals2(self, wb):
        ...
        self.browser.get("https://www.transfermarkt.com/1-bundesliga/eigentorstatistik/wettbewerb/L1")
        ...
        logger.info('Bundesliga Own Goals')
        ...
        for row in dataTable.find_elements_by_css_selector('tr'):
            ...
            for cell in row.find_elements_by_tag_name('td'):
                ...
                if cell.text != '':
                    logger.info(u'cell: '.join(cell.text).encode('utf-8').strip())
                ...
                if ctt < 1:
                    ...
                    if first_column[x].value is None:
                        ...
                        if c[0] in first_column[x].value:  # or \
                            # (c[0] == 'Atl. Madrid' and first_column[x].value == 'Atlético Madrid') or \
                            # (c[0] == 'Ath. Bilbao' and first_column[x].value == 'Athletic Bilbao') or \
                            # (c[0] == 'Alaves' and first_column[x].value == 'Deportivo Alavés') or \
                            # (c[0] == 'Celta Vigo' and first_column[x].value == 'Celta de Vigo') or \
                            # (c[0] == 'Leganes' and first_column[x].value == 'CD Leganés'):
    ...
    def getCorrectScores(self, wb):
        ...
        time.sleep(4)
        self.browser.get(
            "https://docs.google.com/spreadsheets/d/e/2PACX-1vR-EricH6lmbjwzf0GgJBq9SDUtDF0Ew1x4eiNNpvntgAbQ6cMCKWJjFlDeXTUFt8xW_QacOEbvPGwH/pubhtml?gid=78603189&single=true&widget=false&headers=false&chrome=false")
            ...
            for c in teams:
                if ctt < 4:
                    ...
                    if first_column[x].value is None:
                        ...
                        if c[0] in first_column[x].value or \
                                (c[0] == 'Dusseldorf' and first_column[x].value == 'Fortuna Düsseldorf') or \
                                (c[0] == 'FC Koln' and first_column[x].value == '1. FC Köln') or \
                                (c[0] == 'Monchengladbach' and first_column[x].value == 'Borussia Mönchengladbach'):
    ...
    def getMatchesPlayed(self, wb):
        # Add Additional column for Own Goals
        self.browser.get(
            "https://www.transfermarkt.com/1-bundesliga/spieltagtabelle/wettbewerb/L1?saison_id=2019&spieltag=24")
        dataTable = self.browser.find_element_by_xpath('//div[contains(string(), "Table Bundesliga 19/20")]/table')
        teams = []
        hders = []
        logger.info('Bundesliga Matches Played')
        ct = 0
        ...
        for row in dataTable.find_elements_by_css_selector('tr'):
            ...
            if teamStats != '':
                logger.info('Matches Played Stats:  ' + str(teamStats))
                teams.append(teamStats)
            i += 1
            logger.info('Teams: ' + str(teams))
        logger.info('No more rows. Entering Data into Excel...')
                ...
                if ctt < 4:
                    ...
                    if first_column[x].value is None:
                        ...
                        if c[1] in first_column[x].value or \
                                (c[1] == 'F. Düsseldorf' and first_column[x].value == 'Fortuna Düsseldorf') or \
                                (c[1] == 'TSG Hoffenheim' and first_column[x].value == 'TSG 1899 Hoffenheim') or \
                                (c[1] == 'Bor. Dortmund' and first_column[x].value == 'Borussia Dortmund') or \
                                (c[1] == 'E. Frankfurt' and first_column[x].value == 'Eintracht Frankfurt') or \
                                (c[1] == 'Bay. Leverkusen' and first_column[x].value == 'Bayer 04 Leverkusen') or \
                                (c[1] == 'Bor. M\'gladbach' and first_column[x].value == 'Borussia Mönchengladbach'):
                            # If team is in ownGoalStats, add the team stats to table in Excel
                            for stat in c:
                                # mycell= ws.cell(row=rowCount, column=columnCount)
                                # mycell = stat
    ...
    def getConcededPenalties(self, wb):
        # TODO: Changed for V5
        self.browser.get("https://www.statbunker.com/competitions/LeagueTable?comp_id=646")
        dataTable = self.browser.find_element_by_css_selector('table.table')
        # TODO: Added in V5
        time.sleep(3)
        clubs = []
        hders = []
        ...
        logger.info('Bundesliga Conceded Penalties Goals')
        ...
        for row in dataTable.find_elements_by_css_selector('tr'):
            ...
            for cell in row.find_elements_by_tag_name('td'):
                ...
                if cell.text != '':
                    ...
                    teamStats.append(cell.text.rstrip())
            if teamStats != '':
                logger.info('Conceded Stats:  ' + str(teamStats))
            ...
            for c in clubs:
                if ctt < 4:
                    ...
                    if first_column[x].value is None:
                        ...
                        # TODO: Added in V5
                        if c[1] in first_column[x].value or \
                               (c[1] == '1. FC Koln' and first_column[x].value == '1. FC Köln') or \
                                (c[1] == '1. FC Union Berlin' and first_column[x].value == '1.FC Union Berlin') or \
                                (c[1] == 'Bayer Leverkusen' and first_column[x].value == 'Bayer 04 Leverkusen') or \
                                (c[1] == 'AS Saint-Etienne' and first_column[x].value == 'AS Saint-Étienne') or \
                                (c[1] == 'Eintracht Frankfurt' and first_column[x].value == 'Eintracht Frankfurt') or \
                                (c[1] == 'Borussia M\'gladbach' and first_column[x].value == 'Borussia Mönchengladbach') or \
                                (c[1] == 'Fortuna Dusseldorf' and first_column[x].value == 'Fortuna Düsseldorf') or \
                                (c[1] == 'Hertha BSC' and first_column[x].value == 'Hertha BSC') or \
                                (c[1] == '1. FSV Mainz 05' and first_column[x].value == '1.FSV Mainz 05') or \
                                (c[1] == 'SC Paderborn 07' and first_column[x].value == 'SC Paderborn 07'):
                            # If team is in clubs, add the team stats to table in Excel
                            for stat in c:
                                # TODO: Added in V5
                                if stat == '-':
                                    logger.info('stat[count]: ' + str(stat))
                                    logger.info('Row/Column: ' + str(x) + ' ' + str(columnCount))
                                    ws1.cell(row=x + 1, column=columnCount).value = 0
                                    logger.info('Cell value: ' + str(ws.cell(row=x + 1, column=columnCount).value))
                                else:
                                    logger.info('stat[count]: ' + str(stat))
                                    logger.info('Row/Column: ' + str(x) + ' ' + str(columnCount))
                            ...
                            logger.info('Cell value: ' + str(ws.cell(row=x + 1, column=columnCount).value) + ' not matching first column: ' + str(first_column[x].value))
                        rowCount += 1
                        ctt += 1
        ...
        wb.save(filenameStats)
        # statsBook.save(filenameStats)
        self.getRefs("https://www.transfermarkt.com/bundesliga/schiedsrichter/wettbewerb/L1/saison_id/2019/plus/1", 0)
        # self.getConcededPenalties(wb)
        # sys.exit()
    ...
    def getRefDetails(self, url):
        ...
        childElement1 = self.browser.find_elements_by_xpath('//img[@alt="Bundesliga"]')
    ...
    def getRefs(self, url, pgCount):
        ...
        refs = []
        hders = []
        logger.info('Bundesliga Refs')
        ct = 0
        count = 0

        wsheet.cell(row=3, column=1).value = 'Referee'
        wsheet.cell(row=3, column=2).value = 'Team'
        ...
        for row in dataTable.find_elements_by_css_selector('tr'):
            ...
            for cell in row.find_elements_by_tag_name('td'):
                logger.info('Count - i: ' + str(i))
                if cell.text != '':
                    logger.info('cell: ' + str(cell.text.rstrip()))
                    logger.info('Count - count: ' + str(count))
...
# TODO: Change for each league
file_name = 'Bundesliga' + d.now().strftime('%Y_%m_%d_%H_%M%S') + '.log'
