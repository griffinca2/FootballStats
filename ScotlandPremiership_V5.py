...
pageCount = 0
filenameStats = 'ScotlandPremiership_V5_' + d.now().strftime('%Y_%m_%d_%H_%M%S') + '.xlsx'
statsBook = Workbook()
wsheet = statsBook.create_sheet('Scotland Premiership Refs')
ws = statsBook.active

wsheet.cell(row=1, column=1, value='Scotland Premiership')
...
class footballStatsScraper:
    ...
    def getGoalDistribution(self):
        self.browser.get("https://www.transfermarkt.com/scottish-premiership/torverteilungart/wettbewerb/SC1/plus/1")
        dataTable = self.browser.find_element_by_css_selector('table.items')
        clubStats = []
        clubs = []

        logger.info('Scottish Premiership Goal Distribution')
    ...
    def getOwnGoals2(self, wb):
        ...
        self.browser.get("https://www.transfermarkt.com/scottish-premiership/eigentorstatistik/wettbewerb/SC1")
        ...
        logger.info('Scottish Premiership Own Goals')
    ...
    def getCorrectScores(self, wb):
        ...
        time.sleep(4)
        self.browser.get(
            "https://docs.google.com/spreadsheets/d/e/2PACX-1vTZ7iAtmuZzQ2iL6NChVQuXeOl8xsft5aVsoluk-gMxb8DyLJ5OuY-QmiNvj6R3GGpMz7fkXnAaVNGv/pubhtml?gid=656804182&single=true&widget=false&headers=false&chrome=false")
                ...
                if ctt < 4:
                    ...
                    if first_column[x].value is None:
                        ...
                        if c[0] in first_column[x].value or \
                                (c[0] == 'Hearts' and first_column[x].value == 'Heart of Midlothian FC') or \
                                (c[0] == 'Hamilton' and first_column[x].value == 'Hamilton Academical FC'):
    ...
    def getMatchesPlayed(self, wb):
        # Add Additional column for Own Goals
        self.browser.get("https://www.transfermarkt.com/scottish-premiership/spieltagtabelle/wettbewerb/SC1")
        # dataTable = self.browser.find_element_by_xpath('//div[contains(string(), "Table Premier League 19/20")]/table')
        dataTable = self.browser.find_element_by_xpath(
            '//div[contains(string(), "Table Scottish Premiership 19/20")]/table')

        teams = []
        hders = []
        logger.info('Scottish Premiership Matches Played')
        ct = 0
                ...
                if ctt < 4:
                    ...
                    if first_column[x].value is None:
                        ...
                        if c[1] in first_column[x].value or \
                                (c[1] == 'Heart of Midl.' and first_column[x].value == 'Heart of Midlothian FC') or \
                                (c[1] == 'Hamilton Acad.' and first_column[x].value == 'Hamilton Academical FC'):
                            # If team is in ownGoalStats, add the team stats to table in Excel
                            for stat in c:
                                # mycell= ws.cell(row=rowCount, column=columnCount)
                                # mycell = stat
    ...
    def getConcededPenalties(self, wb):
        self.browser.get("https://www.statbunker.com/competitions/LeagueTable?comp_id=643")
        ...
        logger.info('Scotland Premiership Conceded Penalties Goals')
            ...
            for c in clubs:
                if ctt < 4:
                    ...
                    if first_column[x].value is None:
                        ...
                        # TODO: Added in V5
                        if c[1] in first_column[x].value or \
                                (c[1] == 'St Mirren' and first_column[x].value == 'St. Mirren FC') or \
                                (c[1] == 'St Johnstone' and first_column[x].value == 'St. Johnstone FC'):
                            # If team is in ownGoalStats, add the team stats to table in Excel
                            for stat in c:
                                # TODO: Added in V5
                                if stat == '-':
        ...
        startingLink = "https://www.transfermarkt.com/scottish-premiership/schiedsrichter/wettbewerb/SC1/saison_id/2019/plus/1"
        self.getRefs(startingLink, 0)
    ...
    def getRefDetails(self, url):
        ...
        childElement1 = self.browser.find_elements_by_xpath('//img[@alt="Scottish Premiership"]')
    ...
    def getRefs(self, url, pgCount):
        ...
        refs = []
        hders = []
        logger.info('Scottish Premiership Refs')
        ct = 0
        count = 0

        wsheet.cell(row=3, column=1).value = 'Referee'
        wsheet.cell(row=3, column=2).value = 'From'
...
file_name = 'ScotlandPremiership_' + d.now().strftime('%Y_%m_%d_%H_%M%S') + '.log'
