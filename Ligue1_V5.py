...
pageCount = 0
filenameStats = 'Ligue1_V5_' + d.now().strftime('%Y_%m_%d_%H_%M%S') + '.xlsx'
statsBook = Workbook()
wsheet = statsBook.create_sheet('Ligue 1 Refs')
ws = statsBook.active


wsheet.cell(row=1, column=1, value='Ligue 1')
...
class Ligue1:
    ...
    def getGoalDistribution(self):
        self.browser.get("https://www.transfermarkt.com/ligue-1/torverteilungart/wettbewerb/FR1/plus/1")
        dataTable = self.browser.find_element_by_css_selector('table.items')
        clubStats = []
        clubs = []

        logger.info('Ligue 1 Goal Distribution')
    ...
    def getOwnGoals2(self, wb):
        ...
        self.browser.get("https://www.transfermarkt.com/ligue-1/eigentorstatistik/wettbewerb/FR1")
    ...
    def getCorrectScores(self, wb):
        ...
        time.sleep(4)
        self.browser.get(
            "https://docs.google.com/spreadsheets/d/e/2PACX-1vScFsZT1LHB58opqMJkGoiLC-mgSUE7XPhDWi9EligYYdHF1ev4yUx6N1UVVnGO7_9yLfv1K3ssKEIB/pubhtml?gid=78603189&single=true&widget=false&headers=false&chrome=false")
            ...
            for c in teams:
                if ctt < 4:
                    ...
                    if first_column[x].value is None:
                        ...
                        if c[0] in first_column[x].value or \
                                (c[0] == 'PSG' and first_column[x].value == 'Paris Saint-Germain') or \
                                (c[0] == 'St. Etienne' and first_column[x].value == 'AS Saint-Étienne') or \
                                (c[0] == 'Rennes' and first_column[x].value == 'Stade Rennais FC') or \
                                (c[0] == 'Nimes' and first_column[x].value == 'Nîmes Olympique'):
    ...
    def getMatchesPlayed(self, wb):
        # Add Additional column for Own Goals
        self.browser.get("https://www.transfermarkt.com/ligue-1/spieltagtabelle/wettbewerb/FR1")
        dataTable = self.browser.find_element_by_xpath('//div[contains(string(), "Table Ligue 1 19/20")]/table')
        teams = []
        hders = []
        logger.info('Ligue 1 Matches Played')
        ct = 0
                ...
                if ctt < 4:
                    ...
                    if first_column[x].value is None:
                        ...
                        if c[1] in first_column[x].value or \
                                (c[1] == 'PSG' and first_column[x].value == 'Paris Saint-Germain') or \
                                (c[1] == 'St. Etienne' and first_column[x].value == 'AS Saint-Étienne') or \
                                (c[1] == 'G. Bordeaux' and first_column[x].value == 'FC Girondins Bordeaux') or \
                                (c[1] == 'R. Strasbourg' and first_column[x].value == 'RC Strasbourg Alsace') or \
                                (c[1] == 'Paris SG' and first_column[x].value == 'Paris Saint-Germain'):
                            # If team is in ownGoalStats, add the team stats to table in Excel
                            for stat in c:
                                # mycell= ws.cell(row=rowCount, column=columnCount)
                                # mycell = stat
    ...
    def getConcededPenalties(self, wb):
        # TODO: Changed for V5
        self.browser.get("https://www.statbunker.com/competitions/AgainstPenalty?comp_id=647")
        ...
        logger.info('Ligue 1 Conceded Penalties Goals')
        ...
        for row in dataTable.find_elements_by_css_selector('tr'):
            ...
            for cell in row.find_elements_by_tag_name('td'):
                ...
                if cell.text != '':
                    ...
                    teamStats.append(cell.text.rstrip())
            if teamStats != '':
                logger.info('Conceded Penalties Stats:  ' + str(teamStats))
        ...
        for x in range(len(first_column)):
            ...
            for c in clubs:
                if ctt < 4:
                    ...
                    if first_column[x].value is None:
                        ...
                        # TODO: Added in V5
                        if c[0] in first_column[x].value or \
                                (c[0] == 'Stade Rennes' and first_column[x].value == 'Stade Rennais FC') or \
                                (c[0] == 'Amiens SC' and first_column[x].value == 'SC Amiens') or \
                                (c[0] == 'Girondins De Bordeaux' and first_column[x].value == 'FC Girondins Bordeaux') or \
                                (c[0] == 'AS Saint-Etienne' and first_column[x].value == 'AS Saint-Étienne') or \
                                (c[0] == 'Dijon FCO' and first_column[x].value == 'FCO Dijon') or \
                                (c[0] == 'Montpellier HSC' and first_column[x].value == 'HSC Montpellier') or \
                                (c[0] == 'Lille OSC' and first_column[x].value == 'LOSC Lille') or \
                                (c[0] == 'Nimes Olympique' and first_column[x].value == 'Nîmes Olympique') or \
                                (c[0] == 'Olympique Lyonnais' and first_column[x].value == 'Olympique Lyon') or \
                                (c[0] == 'Angers SCO' and first_column[x].value == 'SCO Angers'):
                            # If team is in ownGoalStats, add the team stats to table in Excel
                            for stat in c:
                                # TODO: Added in V5
                                if stat == '-':
            ...
            for row in range(min_row, max_row):
                for col in range(min_col, max_col):
                    if ws1.cell(row=row, column=col).value is None:
                        if col == min_col:
                            ws1.cell(row=row, column=col).value = ws1.cell(row=row, column=1).value
                            logger.info('Adding ' + str(ws.cell(row=x + 1, column=1).value))
                        else:
                            ws1.cell(row=row, column=col).value = 0
                            logger.info('Adding 0 to ' + str(min_row) + str(min_col))
        ...
        # self.getConcededPenalties(wb)
        # sys.exit()
        self.getRefs('https://www.transfermarkt.com/ligue-1/schiedsrichter/wettbewerb/FR1/saison_id/2019/plus/1', 0)
    ...
    def getRefDetails(self, url):
        ...
        childElement1 = self.browser.find_elements_by_xpath('//img[@alt="Ligue 1"]')
