...
pageCount = 0
filenameStats = 'EnglishChampionshipLeague_V5_' + d.now().strftime('%Y_%m_%d_%H_%M%S') + '.xlsx'
statsBook = Workbook()
wsheet = statsBook.create_sheet('English Championship League Refs')
ws = statsBook.active


wsheet.cell(row=1, column=1, value='English Championship League')
...
class EnglishChampionshipLeague:
    def __init__(self):
        self.refNum = 0
    ...
    def getGoalDistribution(self):
        self.browser.get("https://www.transfermarkt.com/championship/torverteilungart/wettbewerb/GB2/plus/1")
        dataTable = self.browser.find_element_by_css_selector('table.items')
        clubStats = []
        clubs = []

        logger.info('English Championship League Goal Distribution')
    ...
    def getOwnGoals2(self, wb):
        ...
        self.browser.get("https://www.transfermarkt.com/championship/eigentorstatistik/wettbewerb/GB2")
        ...
        logger.info('English Championship League Own Goals')
    ...
    def getCorrectScores(self, wb):
        ...
        time.sleep(4)
        self.browser.get(
            "https://docs.google.com/spreadsheets/d/e/2PACX-1vSM6PY5xq0v455cdr30HFUtxZfqwas8QqY1a4_ixGZAT4XiS5Hi5idYWfud-5rn8FdGRrnS4sTQaDYK/pubhtml?gid=78603189&single=true&widget=false&headers=false&chrome=false")
            ...
            for c in teams:
                if ctt < 4:
                    ...
                    if first_column[x].value is None:
                        ...
                        if c[0] in first_column[x].value or \
                                (c[0] == 'QPR' and first_column[x].value == 'Queens Park Rangers') or \
                                (c[0] == 'Stoke' and first_column[x].value == 'Stoke City') or \
                                (c[0] == 'West Brom' and first_column[x].value == 'West Bromwich Albion') or \
                                (c[0] == 'Nottm Forest' and first_column[x].value == 'Nottingham Forest') or \
                                (c[0] == 'Sheff Wed' and first_column[x].value == 'Sheffield Wednesday'):
    ...
    def getMatchesPlayed(self, wb):
        # Add Additional column for Own Goals
        self.browser.get("https://www.transfermarkt.com/championship/spieltagtabelle/wettbewerb/GB2")
        dataTable = self.browser.find_element_by_xpath('//div[contains(string(), "Table Championship 19/20")]/table')
        teams = []
        hders = []
        logger.info('English Championship League Matches Played')
        ct = 0
                ...
                if ctt < 4:
                    ...
                    if first_column[x].value is None:
                        ...
                        if c[1] in first_column[x].value or \
                                (c[1] == 'QPR' and first_column[x].value == 'Queens Park Rangers') or \
                                (c[1] == 'Stoke' and first_column[x].value == 'Stoke City') or \
                                (c[1] == 'West Brom' and first_column[x].value == 'West Bromwich Albion') or \
                                (c[1] == 'Nottm Forest' and first_column[x].value == 'Nottingham Forest') or \
                                (c[1] == 'Sheff Wed' and first_column[x].value == 'Sheffield Wednesday') or \
                                (c[1] == 'Preston NE' and first_column[x].value == 'Preston North End'):
                            # If team is in ownGoalStats, add the team stats to table in Excel
                            for stat in c:
                                # mycell= ws.cell(row=rowCount, column=columnCount)
                                # mycell = stat
    ...
    def getConcededPenalties(self, wb):
        # TODO: Changed for V5
        self.browser.get("https://www.statbunker.com/competitions/LeagueTable?comp_id=640")
        ...
        logger.info('English Championship League Conceded Penalties Goals')
                    ...
                    if first_column[x].value is None:
                        ...
                        if c[1] in first_column[x].value:
                            ...
                            for stat in c:
                                # TODO: Added in V5
                                if stat == '-':
                                    logger.info('stat[count]: ' + str(stat) + ' Row/Column: ' + str(x) + ' ' + str(columnCount))
                                    # logger.info('Row/Column: ' + str(x) + ' ' + str(columnCount))
        ...
        # self.getConcededPenalties(wb)
        # sys.exit()
        self.getRefs('https://www.transfermarkt.com/championship/schiedsrichter/wettbewerb/GB2/saison_id/2019/plus/1', 0)
    ...
    def getRefDetails(self, url):
        ...
        childElement1 = self.browser.find_elements_by_xpath('//img[@alt="Championship"]')
    ...
    def getRefs(self, url, pgCount):
        ...
        refs = []
        hders = []
        logger.info('English Championship Refs')
        ct = 0
        count = 0
...
file_name = 'EnglishChampionship_' + d.now().strftime('%Y_%m_%d_%H_%M%S') + '.log'
